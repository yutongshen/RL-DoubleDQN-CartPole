...
class RL:
    def __init__(self, Feature_size, Actions, Epsilon=.5, Gamma=.9, lr=.01, Memory_size=200, replace_target_iter=200, IsDouble=True):
    ...
    def learn(self):
        ...
        if self.Double:
            index = self.q_evaluation_model.predict(s_batch_).argmax(axis=1)
            max_q = self.q_target_model.predict(s_batch_)[range(len(s_batch)), index]
        else:
